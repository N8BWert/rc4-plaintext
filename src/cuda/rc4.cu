...
// CUDA kernel taht generates the first 8 bytes of a keystream for 64 bit keys with a 256 byte drop_n.
...
// Kuda kernel to generate the first 8 bytes of 1000000 keystreams for 32 bit keys with a drop_n of 0.
extern "C" __global__ void rc4_keystream_gen_32_drop_0(const u8 *key, u8 *out_keystream) {
	u64 write_idx = 0;
	for (u64 iter = 0; iter < 256; iter++) {
		u8 s[256];
		u8 temp;
		for (u16 i = 0; i < 256; i++) {
			s[i] = i;
...
extern "C" __global__ void rc4_keystream_gen_32_drop_256(const u8 *key, u8 *out_keystream) {
	u64 write_idx = 0;
	for (u64 iter = 0; iter < 1000000; iter++) {
		u8 s[256];
		u8 temp;
...
extern "C" __global__ void rc4_keystream_gen_32_drop_267(const u8 *key, u8 *out_keystream) {
	u64 write_idx = 0;
	for (u64 iter = 0; iter < 1000000; iter++) {
		u8 s[256];
		u8 temp;
